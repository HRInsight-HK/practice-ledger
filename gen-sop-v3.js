  ...
  return makeDoc([
    ...
    makeTable(['入口', '地址', '说明'],
      [
        ...
        ['本地SSC工具', 'hr-local.html（本地运行）', '薪资管理，不上云'],
      ], [15, 45, 40]),
    emptyLine(),
  ...
  return makeDoc([
    ...
    makeTable(['入口', '地址'],
      [
        ['管理后台', CLOUD_URL],
        ['问卷表单', Q_URL],
        ['本地SSC工具', 'hr-local.html（本地运行，薪资不上云）'],
      ], [25, 75]),
    emptyLine(),
...
function createAdminSOP() {
  return makeDoc([
    ...
    h2('四、注意事项'),
    warningBox('实操天数>7天时，系统会自动发送用工风险预警@全员，请务必提醒需求方控制天数。'),
  ...
  return makeDoc([
    ...
    makeTable(['入口', '地址', '说明'],
      [
        ...
        ['本地SSC工具', 'hr-local.html（本地运行）', '薪资录入与结算，不上云'],
        ['问卷表单', Q_URL, '转发给需求方'],
      ], [20, 50, 30]),
    emptyLine(),
    ...
    h2('三、核心操作'),
    h3('3.1 薪资录入（本地工具）'),
    p('1. 用浏览器打开 hr-local.html'),
    p('2. 输入云端服务地址：' + CLOUD_URL),
    p('3. 登录（用户名：hr，密码：hr666）'),
    p('4. 设置本地加密口令（保护薪资数据）'),
    p('5. 点击"登记薪资"，填写：'),
    ...
    bullet('薪资备注'),
    emptyLine(),
    warningBox('薪资数据仅存储在本地浏览器中，不会上传到云端！请定期导出备份。'),
    emptyLine(),
    ...
    emptyLine(),
    tipBox('本地SSC工具地址：hr-local.html。云端后台地址：' + CLOUD_URL),
  ], 'SSC操作SOP v3.1');
}
  ...
  return makeDoc([
    ...
    makeTable(['入口', '地址', '使用人', '说明'],
      [
        ...
        ['本地SSC工具', 'hr-local.html（本地运行）', 'SSC(Zoe)', '薪资管理，不上云'],
      ], [15, 40, 20, 25]),
    emptyLine(),
    ...
    bullet('点击"导入登记"，补充设备信息'),
    bullet('如天数>7天，系统自动发送用工风险预警@全员'),
    bullet('登记后系统自动@青霖通知'),
    emptyLine(),

    h3('6.3 薪资录入（SSC Zoe）'),
    bullet('用浏览器打开 hr-local.html（本地运行）'),
    bullet('输入云端地址：' + CLOUD_URL),
    bullet('登录（hr / hr666），设置本地加密口令'),
    bullet('找到对应人员，点击"登记薪资"'),
    bullet('填写薪资结构、基本工资、补贴等'),
    warningBox('薪资数据仅存储在本地浏览器，绝不上云。请定期导出备份。'),
    emptyLine(),
